...
names = ["dim", "size", "total MST weight", "avg edge weight", "max edge weight", "duration (s)"]
...
while (size < 65536):
    for dim in [0,2,3,4]:
        ...
        new_row = pd.DataFrame([dim, size, float(weight[0]), float(weight[1]), float(weight[2]), duration]).T
