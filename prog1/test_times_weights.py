...
names = ["dim", "size", "total MST weight", "avg edge weight", "duration (s)"]
...
while (size < 65536):
    for dim in [0,2,3,4]:
        ...
        new_row = pd.DataFrame([dim, size, float(weight[0]), float(weight[1]), duration]).T
        new_row.columns = names
        times = pd.concat([times, new_row],ignore_index=True)
    print(times)
    print("\n")
    size *= 2
    times.to_csv("weights_durations.csv")
